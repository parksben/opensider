...
/**
 * The popup behind the collapsed composer action button (see ChatPane): the same items the
 * flat row shows, stacked above the plus button. Layout mirrors AttachMenu, which this
 * replaces once the four buttons no longer fit.
 */
export function ComposerActionsMenu({
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      const width = Math.min(Math.max(menu.offsetWidth, MENU_WIDTH), maxWidth);
      const maxHeight = Math.max(96, vh - SAFE * 2);
