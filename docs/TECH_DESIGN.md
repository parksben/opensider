...
- 一轮开始记 `turnStartedAt`；`turn.end` / 停止 / 连接报错结束时，若末尾 assistant 的 `createdAt` 不早于开始时间，写入 `durationMs` 并持久化。`AssistantMessage`：找最后一段 `type=text`，它之前是过程。`live` 仅当该会话 `isRunning` 且该条是末尾 assistant 且还没有 `durationMs`：二次打开历史会话（`runningIds` 不落盘）和刚结束时走同一条 `!live` 折叠路径，不跑 `liveVisibleParts`。落盘保留 reasoning / tool-call / durationMs。`finishTurn` 写 `durationMs` 时把未收尾的 tool `pending` / `in_progress` 收成 `completed`；`hydrateSession` 对已有 `durationMs` 的 assistant 再收一次，避免历史里还转圈。`isRunning` 且该条是最后一条且无 `durationMs` 时不收成耗时行：`liveVisibleParts` 把连续的 `reasoning` / `tool-call`（中间没有 `text`）收成只渲最后一条，同一行被新步骤替换；一旦后面接上 `text`，把这条过程标题也摘掉，只留正文。`live` 且该 `reasoning` 仍是整条 content 末尾时，标题左侧渲 `LoaderCircle`（与工具进行中同色），后面跟了别的 part 或回合结束则去掉。结束后默认收起过程，只留正文；用户展开耗时行时再按原顺序把每一步各占一行。耗时行、思考、工具调用共用 `TextFold`：无底透明全宽 `button`，muted 12px 文案 + 可选 kind 图标 + 紧挨着的箭头（`inline-flex`，不要 `justify-between`）。收起时 `ChevronRight` 仅该行 hover 出现（`group/fold`，避免吃到 `MessageFrame` 的 `group`）；展开后换成 `ChevronDown`。过程区走 `.cs-process-scroll`（只设 `max-height`），思考走 `max-height: 5lh` 的 `.cs-fold-scroll`，工具再叠 `.cs-tool-scroll`（`10lh`）。线程是 `flex-col-reverse` 贴底，展开会长高把灰字顶上去：toggle 前记下 `getBoundingClientRect().top`，`useLayoutEffect` 里给 `.cs-thread` 的 `scrollTop` 加上位移，把灰字钉回原处，看起来是向下展开。滚动盒由 `FadeScroll` 包一层 `isolation: isolate`，上下各一条 `mix-blend-mode` 渐变（深色 `multiply`、浅色 `lighten`，色用 `--ink`）；`scrollTop>2` 才显示顶遮罩，距底 `>2` 才显示底遮罩。没有过程（只有正文）不渲染这一行。
...
- 源文件 `packages/extension/assets/icon.svg` 由 `scripts/generate_icon.py` 生成，勿手改：圆形 + 光标箭头（取自 Cursor 官方 CUBE_2D 内子路径，绕圆心顺时针转 30°）合成单条 compound path 做 clipPath（evenodd 镂空），360 个 1° 扇形逼近 conic 渐变，实现红→黄→绿顺时针风车，交界 40° smoothstep 平滑过渡
