...
“本机装有 Agent”或“该产品有桌面 UI”本身都不满足这两个条件。Claude、Codex 等桌面应用把会话与运行时封装在自己的 UI 和私有进程边界内，没有向第三方浏览器扩展暴露稳定的 ACP 子进程入口。通过辅助功能或坐标点击去遥控桌面 UI，不仅依赖易变的界面结构，也无法可靠映射 ACP 的流式事件、工具状态、权限和会话生命周期，所以不作为接入方案。同品牌的 CLI 是另一套入口：README 对所有产品采用同一套三步引导——从产品官网找到官方 Agent CLI、按官方文档安装并登录、重新执行 OpenSider 安装提示词以完成探测与可用的 ACP 适配。Claude Code 与 Codex 只作为带官方文档链接的示例；安装流程探测到 `claude` / `codex` 后，负责安装或校验对应 ACP 适配器。README 不复制易过期的平台命令，也不让用户手工拼接适配器；没有 CLI、ACP 接口或可用适配器的产品仍明确判定为不可接入。
...
- 缓存最近一次 `status` / `session` / `page` / `models`，侧栏 `onConnect` 或 `ping` 时立即回放，避免 Host 已 ready 但面板因 React StrictMode 重挂而一直显示离线。连接中的空 `models` 不写入缓存（那是还在加载，不是确认空名单）。Host 在 `hello` 且已 `ready` 时再推一次 catalog。**回放和侧栏读本地缓存是赛跑的**（会话多、历史长时 `loadState` 明显更慢），所以「要不要自动连上记得的那家」不能只在收到 `agents` / `idle` 那一刻判一次：回放先到时侧栏的 refs 还全是空的（引导状态没灌进来、`selectedProvider` 也空），判断被跳过就再没人补一次——面板看着正常，模型列表和推理档位却一片空白，要手动切一次 Agent 才醒过来（用户报过；大状态的沙箱能稳定复现）。现在抽成一个 `autoConnectTarget()`（引导已完成 + 记得是哪家或名单里有第一家 + Host 已扫完 Agent 列表 + 状态是 `idle`/`starting`），两条消息分支与 `hydrated` 之后各判一次；`scripts/verify-first-connect.mjs` 守着这条竞态。**判断放在多处就必须幂等**：`agents` / `idle` 两条回放与 hydration 之后那次判定很容易落在同一拍，各发一次 `agent.connect` 会让 Host 把连接一次次 `beginConnect()` 作废重来，最后谁都没连完——面板停在 `connecting`，模型列表与配置项全空（真实报过，沙箱能稳定复现）。所以侧栏额外记一个「连接在飞」标记：发出 `agent.connect` 就置位，收到 `connecting` 之外的状态（`ready` / `error` / `missing` / `starting`）才解除（`idle` 刻意不算——它只会来自 Host 启动或我们自己的取消），在飞期间自动路径不再发。`verify-first-connect.mjs` 里「只发一次」那条断言就是守它的。**“快照里的料”必须能晚到**：卸载重装后扩展本地缓存是空的（`onboardingCompleted` 与记得的 Agent 都空），这两样只能靠 Host 镜像补，而镜像（1MB 上下、分片）往往晚于 hydration 到——所以判定不能只在「消息到达那一刻」或「hydration 完成」各判一次，而是挂在快照本身的那几个 state（`hydrated` / `onboardingCompleted` / `selectedProviderId` / `sawAgents` / `status` / `agents`）上重判。不重判的话，镜像把料补齐之后没人再问一句，首次打开就一直不连（真机日志：hydration 19:44:47、镜像随后落地、直到 19:44:55 才因为一次迟到广播醒过来——用户感觉就是「等好几秒还是空，关掉重开就好了」；沙箱里用 `SCENARIO=mirror` 跑这条路径）。重判靠 `connectInFlightRef` 保持幂等。
...
- **不声明成常驻 `content_scripts`，也不交给 CRXJS 的 ESM loader**。两个钩子由 `packages/extension/scripts/build-page-hooks.mjs` 单独编成**经典脚本**（IIFE、无 import/export、固定名 `page-hooks/native-ui-hook.js` / `page-hooks/activity-hook.js`）。为什么非得这样：CRXJS 给内容脚本生成的是 `assets/xxx-loader-*.js` + `await import("…")`，而**同一个模块在同一个文档里只会求值一次**——面板关掉我们卸载钩子之后，再注入就是空转，Agent 在页面刷新前一直失明（实测：`getNativeUi` 直接报「could not be installed」）；loader 还是异步的，`executeScript` 返回时钩子尚未在听，紧接着的那次调用会把钩子装成「没有 caller token」的状态。经典脚本两个问题一起没了（固定名也顺带干掉了原来那条「登记进 manifest 只为回读 hash 文件名」的 `exclude_matches` 写法）。
- **按需注入**：`background.ts` 的 `injectPageHook(tabId, name)` 用 `chrome.scripting.executeScript({ target: { tabId, allFrames: true }, world: "MAIN", files: ["page-hooks/…js"] })` 注入，经典脚本在 `executeScript` resolve 时就已落位；钩子自身的 `if (host[KEY]) return` 保证重复注入幂等。触发点是「侧栏开着 + Agent 触及该标签」：`port.onConnect`（面板打开时的活动标签）、`dispatchCommand`（每条命令都先 `armActivity`，读完原生 UI 事件后按需补注入 native UI 钩子）、`runNativeUiMethod`（`getNativeUi` / `setDialogPolicy` 不再报「reload the tab」，而是先补注入再重试）。
- **为什么不再靠 `run_at: document_start`**：内容脚本是经典脚本，CRXJS 会把它们编成 loader + `await import("…")`，真正的代码要等模块 fetch + 编译。实测（改之前）页面自身首个 `<script>` 已经在 ~18ms 跑完，钩子到 ~39ms 才落地，等于在页面初始化**进行到一半**时把 API 换掉，比 `document_start` 更糟。按需注入没有这个尴尬：装上时页面早就初始化完了，我们只在「Agent 要动手」的语义下改环境。现在钩子是经典脚本，真要回到早注入也已经能真「早」了。
- **感知不受影响**：`browser/tabs.json` / `current.json` / 「当前活动标签」全部来自 SW 的 `chrome.tabs` / `chrome.windows`，以及隔离世界的内容脚本；主世界注入范围收窄不会动到它们。
- **未触及的页面必须逐项一致**：`scripts/verify-page-tamper.mjs` 把「什么都别动」变成 CI 能查的断言（未触碰页面 0 个非原生入口 / 0 个全局 / `Object.getOwnPropertyNames(HTMLDocument.prototype) === ["constructor"]`；触及后钩子到位；释放后还原到与未触碰时**完全一致**；再触及能第二次装上；页面抢走钩子时扩展自愈）。
- **释放即卸载**：`releaseActivity()`（侧栏收起 / 最后一个面板端口断开 / 扩展重置）除了解除武装，还会调 `releasePageHooks()`：两个钩子把包过的入口换回捕获到的原函数、`EventTarget.prototype` 与 `document` 原型上的补丁移除、并把各自挂在 `window` 上的全局删掉（所以是 `configurable: true`）。因为钩子是经典脚本，之后再次触及会**重新执行**并重新装上。
- **caller token 与自愈**：钩子里的 token 认领逻辑收在 `packages/extension/src/hook-caller.ts`（单测覆盖）——`handshake` 把「没装」「被页面拿了」「就是你」三件事分清楚（第一个长得像 token 的调用者拿到它；`release` 永远不认领）。SW 每次调用前先 `handshake`，拿到 `authorized: false` 就知道页面抢了钩子，于是 `reseatPageHook()`：删掉旧实例 + 重新注入，下一次调用即可认领（有界：每次调用最多重新装一回，不会被来回拖住）。代价写在明处：页面抢走的那一层包装会留在页面上（原函数在它的闭包里，我们无法替它还原），行为仍是透传；活动态那层补丁属于我们持有的实例，照旧能还原。
...
1. **发现**：Host 侧新增 `internal/skills`，扫六个全局目录（`~/.claude/skills`、`~/.cursor/skills`、`~/.codex/skills`、`~/.agents/skills`、`~/.cursor/skills-cursor`、`~/.stepclaw/skills`），每个子目录找 `SKILL.md`，**只读文件头部**（8KiB 上限）解析 frontmatter 的 `name` / `display_name` / `description`（要处理 `>-` / `|` 折叠块与引号）。纯文件 IO、不起子进程：本机 36 个 skill 的实测总耗时在毫秒级，所以允许按需重扫（列表超 5 分钟由侧栏发 `skills.refresh`）。
...
模式是**会话级**的（`session/new|load` 的返回里才带），而侧栏绑会话是它自己的节奏（连接就绪、切会话、发消息都会触发），所以**不能把「模式可见」挂在侧栏那次绑定上**：`connectAgent` 成功后就由 Host 预建一个会话（`runtime.prepared`）并把模式推出去，侧栏随后的 `session.new` 直接**采用**它（`takePrepared`：`session.new` 无条件采用；`session.use` 只在 id 相同时采用，否则丢弃；`session.fork` 永不采用，必须真的 fork）——不预建的话，连接/切换 Agent 后模式下拉要等用户发第一条消息才出现（真实出现过的现象）。`requestConnect` 的回滚分支（`statusRef.current = "ready" / "idle"`）那种「消息处理器里先把新状态写进 ref」的写法也要遵守：`tryBindCurrent()` 读的是 ref。以前全仓库没人处理 `current_mode_update`，Agent 自己换了模式 UI 还停在旧值。用户的选择按 **Agent id** 记在 `chrome.storage.local`（与 `selectedModelId` 同路子，并一起镜像进 `ui-state.json`）；下个会话若仍在广告集合内就沿用，否则回到 Agent 的默认。切换 Agent 时清空。
...
| ——（配置项实况，供上面这张表参考） | | Claude `effort [thought_level]` + `fast [model_config] boolean`；Cursor `fast [model_config] select`；Copilot / OpenCode **无** |

### 其它配置项（思考档位 / `model_config`）

`mode` / `model` 之外，规范还允许引擎广告别的配置项。我们只认**两类有明确定义的类别**，其余一律不画（只记日志）——类别是 UX 元数据，规范要求客户端**缺省时也能优雅降级**，各家不一定发、发了也不一定同名：

| 类别 | 处理 |
|---|---|
| `thought_level` | 输入栏里**模型下拉右侧**画一个 pill（引擎自己的推理档位，Claude 是 `Effort`） |
| `model_config` | **不占工具栏**，列进模型下拉菜单里的一个小节（Cursor 的 `Fast`、Claude 的 `Fast mode`） |
| 其它 | 不渲染，仅在收到时记一条日志 |

解析统一走 `internal/sessioncfg`：把 `session/new|load|fork` 返回的 `configOptions` 一次解析成 `[]Option{ID, Category, Name, Type, Current, Values[]}`——**保持数组顺序**（规范说顺序即优先级，所以同类多项只取第一项）；`options[]` 允许 `{group, options}` 分组，**必须摊平**；值取 `value`/`id` + `name`，布尔项的值就是 `true`/`false`。`internal/modes` 不再自己解析，改为消费它的结果：一套解析只留一处，`modes` 那批基于真机 payload 的单测就是这条重构的回归网。
...
**少于两项不画**（一项等于没得选），与模式控件同一条规矩。**不猜**：引擎不广告就没有——Copilot CLI 1.0.86 的 ACP 至今只发 `mode` + `allow_all`，OpenCode 只发 `model` + `mode`；硬造一个档位下拉只会白发 RPC。

**渲染**：`AgentOptionSelect`（思考档位钮）与模式 / 权限下拉同构（同一套弹出层、涟漪、`bottom-full` 定位），但**不带图标**：档位名（`Xhigh` / `Medium`）就是用户唯一要看的信息，图标只会跟另外两个下拉抢地方、还把标签挤成硬切；名称用引擎的 `name`（不翻译，只首字母大写），菜单项用引擎给的值名（`Low` / `High` / `Max`…），当前值打 `Check`，tooltip 给「选项名 — 当前值名」。`model_config` 那部分在 `ModelSelect` 菜单底部画一个分组：选项名 + 值名，当前值打 `Check`，点选即设（布尔项按 `Off` / `On` 两项列）。
...
- **只留结论**：过程叙述不该落到结果层。**搜索整轮都不推增量**——Agent 会先说一段「我先去搜一下…」、两次搜索之间还会再插一段中间正文，这些都会先出现、再被最终正文刷掉（用户报过），所以只攒着，这一轮结束时一次性给结论，那段时间结果层老实停在「搜索中」。累积以**工具调用**为界：收到 `tool_call` 就把已累积的正文挪作兜底、清空累积，最后交出去的是最后一次调用之后的正文（工具调用之后一个字都没说的极端情况才退回被清掉的那段，宁可给过程，也别给一层空白）；没有任何工具调用时就整段用，把「只要结论」交给提示词。翻译没工具可调、正文也短，照常逐字流（结果层立刻有反馈）。
...
- 设置 tab 展示「扩展版本 / 桥接版本 / 最新版本」+「检查更新」+「一键卸载」，两个按钮文案居中。**桥接版本行取自 `hello.version`**（每次连上必发），不依赖 `release` 消息——版本检查失败时那条可能永不到达，此前「本地桥接程序一 直显示未知」就是这么来的。最新版本由 Host 查 `https://api.github.com/repos/parksben/opensider/releases/latest`（未认证 60 次/时/IP），**被限流或不可达时退回网页端 `/releases/latest` 的 302（带 no-cache）**——共享出口 IP 很容易把 API 配额打满（2026-09-18 的「检查失败」事故），网页端不受这个配额限制；两条路都失败才用陈旧缓存并标 `stale`（手点检查会如实报 `failed`，不谎报「已是最新」）。结果缓存到 `~/.opensider/release-check.json`，**TTL 1h**（再长了会出现「刚发完新版，侧栏一天内还说最新是旧的」）；开机检查失败静默降级——不提示、不打扰、不阻塞任何功能。「检查更新」发 `release.check`，Host 强制重查并推 `release`。扩展侧把这轮手点的检查建模成 `ReleaseCheckState`（`idle` / `checking` / `current` / `failed`，见 `version.ts`）：点击后进入 `checking` 并起一个 8s 看门（离线时不会永远卡住）；收到 `release` 时若 `release.check` 是用户手点的（`checkPendingRef`），比一下版本——有新版本直接把 `UpdateDialog` 弹出来（不用用户再去找顶栏图标），没有则 `current`，看门狗超时或收到 `stale` 则 `failed`，两者都亮 1.5s 后回 `idle`。开机时 Host 自己推的那次检查不弹窗、不闪提示（`checkPendingRef` 为假）。
