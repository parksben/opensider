...
- 工具行左侧按 ACP `kind` 换 lucide 图标：read / edit / execute / search / fetch 等，颜色跟灰字走。`pending` / `in_progress` 时只渲 `LoaderCircle`，结束后才换回 kind 图标，二者不同时出现。进行中标题用 `toolLiveHeadline`：短名（`toolLabel`）+ 全角 `：` + `toolPrimaryArg`（按 kind 优先取 `url` / `path` / `command` / `query` 等，没有再退到 title 里解析出的路径）。`TextFold` 一行：`flex w-full min-w-0`，文案 `flex-1 truncate whitespace-nowrap`，箭头 `shrink-0`，超出省略不折行。结束后仍走原来的 `toolTitle`。
