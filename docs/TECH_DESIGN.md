...
- 会话列表是遮罩模态（无关闭钮、无底栏）：遮罩 `fixed inset-0` + `--overlay` + `backdrop-blur-md`（与拾取遮罩同一档，避免后面聊天抢视觉）。顶行左 `MessageSquarePlus` + 新建文案、右无边框筛选；列表按 `updatedAt` 倒序，标题包含匹配。面板 `max-h-[calc(100dvh-400px)]` 上下各留 200px，列表内部滚。`ArrowUp`/`ArrowDown` 循环高亮并 `scrollIntoView({ block: "nearest" })`，`Enter` 切换并关闭，Esc / 点遮罩关闭。卡片上 Pencil / Trash2；`titleManual` 为真时不再用首条消息改标题
