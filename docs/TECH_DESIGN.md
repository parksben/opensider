...
- 空会话：消息区垂直居中，`<img>` 引用 `packages/extension/assets/icon.svg`（`?url` 打包），约 120×120、`opacity-20`，装饰图 `alt=""` / `aria-hidden`；下面一行淡灰提示。不再用 Lucide `MessageSquareMore`。提示 `w-full` + `padding-inline: min(200px, max(1rem, 50% - 12rem))`：宽时两侧约 200px、一句不折；窄侧栏再收 padding 并允许换行
