...
历史标签存在 `opensider/tab-history`：侧栏自己听 `chrome.tabs`（onCreated / onUpdated / onActivated）并启动时 `query`，按完整 URL 去重、最近见过的在前。`dedupeHistoryTabs` 在 load / merge / save / 菜单渲染前都跑一遍：同 URL 只留一条；若传入当前存活 `tabId` 集合，优先保留仍打开的那条，其余同 URL 记录丢掉。关标签不删「该 URL 最后一条」历史。菜单第一 tab 用这份列表（左 favicon，失败则 Lucide `Globe`）。
