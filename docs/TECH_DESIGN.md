...
- 一轮开始记 `turnStartedAt`；`turn.end` / 停止 / 连接报错结束时，若末尾 assistant 的 `createdAt` 不早于开始时间，写入 `durationMs` 并持久化。`AssistantMessage`：找最后一段 `type=text`，它之前是过程。`isRunning` 且该条是最后一条时全展开；结束后默认收起过程，只留正文。收起行是无底、无涟漪的全宽透明 `button`（方便点整行），里面只有左对齐的 muted 文案 + 紧挨着的 lucide 箭头（`inline-flex`，不要 `justify-between`）。收起时 `ChevronRight` 仅 hover 出现；展开后换成 `ChevronDown`。点开后过程放进 `max-h-[min(36vh,16rem)]` 的滚动盒，避免把正文顶出视口。线程是 `flex-col-reverse` 贴底，展开会长高把灰字顶上去：toggle 前记下 `getBoundingClientRect().top`，`useLayoutEffect` 里给 `.cs-thread` 的 `scrollTop` 加上位移，把灰字钉回原处，看起来是向下展开。滚动盒 `isolation: isolate`，上下各一条 `mix-blend-mode` 渐变（深色 `multiply`、浅色 `lighten`，色用 `--ink`）；`scrollTop>2` 才显示顶遮罩，距底 `>2` 才显示底遮罩。没有过程（只有正文）不渲染这一行。
