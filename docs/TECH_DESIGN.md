...
- 一轮开始记 `turnStartedAt`；`turn.end` / 停止 / 连接报错结束时，若末尾 assistant 的 `createdAt` 不早于开始时间，写入 `durationMs` 并持久化。`AssistantMessage`：找最后一段 `type=text`，它之前是过程。`isRunning` 且该条是最后一条时全展开；结束后默认收起过程，只留正文。收起行是 `RippleButton`：左 muted 耗时文案，右 lucide `ChevronRight`（仅 hover 出现）；展开后换成始终可见的 `ChevronDown`。没有过程（只有正文）不渲染这一行。
