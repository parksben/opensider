...
docs/demo/            README 用的侧栏演示视频（H.264 MP4）。`opensider.mp4` 是 The Verge → 今日最热 AI 新闻 HTML → Agent 在本窗口打开 → 人手滚动预览（2880×1800，14.3s，不裁 16:9；原 4–31s 压成 6s），分镜在 `NEXT-TAKE.md`。打字可 setpts 加速，发送前空等硬切，Agent 流式/等待 4x–6x，新标签/产物/打开简报 1x–2x；BGM 用已下载的 CC 曲、始终 1x（见 MUSIC.md）
