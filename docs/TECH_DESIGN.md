...
- 顶栏从左到右：左簇为连接状态（`status !== "ready"` 才画 `Unplug` + 文案；`ready` 不画「已连接」）和 favicon，出错时再跟「Connection / 重连」。中簇绝对居中：标题 + 无边框 `Pencil`（编辑中换成 `Check`）；点铅笔把标题换成 `input`，回车或点对勾 `onRename`，Esc 取消。`max-width` 仍按左右簇各留 32px 用 `ResizeObserver` 算。右簇：`MessageSquarePlus`（直接 `newSession`，不打开列表）→ `History`（开关 `SessionModal`，tooltip「历史会话 / Past chats」）→ 开关灯 → 语言（「中」/「EN」）。favicon 两行 tooltip；取不到用 `Globe`。顶栏按钮不加涟漪。`SessionModal` 顶行只剩全宽筛选，不再放新建。
