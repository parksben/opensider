...
7. 定位元素可用 CSS 选择器，或可见文本（可再加 `nth`）。点击前滚动到元素并短暂高亮。
8. 填写控件时派发 input/change，以便 React / Vue 等受控输入能收到。
9. 导航只允许 `http(s)`。操作完成后刷新当前页快照。
10. 以下页面不注入、不抓取、不操作：`chrome://`、`chrome-extension://`、Chrome Web Store。
11. 页面自动化默认执行，不再逐步弹权限（用户装这个扩展就是为了让 Agent 动手）。本地文件写入和 Shell 仍走 ACP 权限条。
