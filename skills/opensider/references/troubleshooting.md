...
|---|---|---|
| "Manifest file is missing or unreadable" | the user selected the wrong folder (the parent, or a nested folder) | point them at the folder that **directly** contains `manifest.json` (`~/.opensider/extension`) |
...
| the folder is invisible in the picker | `~/.opensider` is a hidden folder | macOS `⌘⇧G` → paste the path; Windows/Linux paste the absolute path |
